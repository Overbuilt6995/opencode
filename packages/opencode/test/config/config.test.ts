...
const wellKnown = (input: {
  ...
  const service = Layer.mock(AuthWellKnown.Service)({
    configs: () =>
      Effect.sync(() => {
        const docs: {
          url: string
        ...
        const base = (input.authUrl ?? "https://example.com").replace(/\/+$/, "")
        const source = `${base}/.well-known/opencode`
        seen.wellKnown = source
        const replaceToken = (text: string) => text.replace(/\{env:TEST_TOKEN\}/g, "test-token")
        if (metadata && typeof metadata === "object") {
          const config = (metadata as { config?: unknown }).config
          if (config && typeof config === "object") {
            docs.push({ url: base, source, dir: path.dirname(source), content: config })
          }
          const remoteConfig = (metadata as { remote_config?: unknown }).remote_config
          if (remoteConfig && typeof remoteConfig === "object" && typeof (remoteConfig as { url?: unknown }).url === "string") {
            const url = replaceToken((remoteConfig as { url: string }).url)
            seen.remote = url
            const headers = (remoteConfig as { headers?: unknown }).headers
            if (headers && typeof headers === "object") {
              const authorization = (headers as { Authorization?: unknown; authorization?: unknown }).Authorization
              const fallback = (headers as { authorization?: unknown }).authorization
              const hit = typeof authorization === "string" ? authorization : typeof fallback === "string" ? fallback : undefined
              if (hit) seen.authorization = replaceToken(hit)
            }
            const remote =
              input.remote &&
              typeof input.remote === "object" &&
              (input.remote as { config?: unknown }).config &&
              typeof (input.remote as { config?: unknown }).config === "object"
                ? (input.remote as { config: unknown }).config
                : input.remote
            docs.push({ url, source: url, dir: path.dirname(url), content: remote })
          }
        }
        return docs
      }),
...
test("remote well-known config can load from AuthWellKnown service", async () => {
  ...
  await provideTmpdirInstance(
    ...
    { git: true },
  ).pipe(Effect.scoped, Effect.provide(configLayer({ wellKnown: fakeAuthWellKnown })), Effect.runPromise)
})
